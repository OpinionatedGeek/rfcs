...
The market maker is a background service so presents no user interface itself other than logging. It is provided as a container, so will typically be run using podman, docker or kubernetes as the underlying platform on a one-container - one-market basis.

This containerization allows the code to encapsulate all dependencies (e.g. node version, all libraries, serum-js version) into one package, as well as provide common mechanisms for upgrading container versions, stopping/starting services, and spawning new container instances in the event of failures.
